...
def test_successful_finish_contract(tmp_db: Any, monkeypatch: Any) -> None:
    ...
    assert "Commit created: True" in res_mcp_a
    assert "Commit message: `feat(phase-one): Refactor auth [t-1]`" in res_mcp_a
    assert "Phase moved to review_pending: False" in res_mcp_a
    assert "Memory review outcome:" not in res_mcp_a
    assert res_mcp_a.count("## Next action") == 1
    assert "Stop here. The active task is finished. Await further instructions." in res_mcp_a
    ...
    assert "Commit created: True" in res_mcp_b
    assert "Commit message: `test(phase-one): Write tests [t-2]`" in res_mcp_b
    assert "Phase moved to review_pending: True" in res_mcp_b
...
def test_finish_verification_gate_e2e_contract(tmp_db: Any, monkeypatch: Any) -> None:
    ...
    assert "Commit created: True" in res_success
    assert "Commit message: `feat(phase-one): Verification gate task [t-gate]`" in res_success
    assert "Phase moved to review_pending: True" in res_success
