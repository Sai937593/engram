...
Inspect current workflow state with `engram_workflow_status`, then execute one scoped Engram task using the simplified MVP task loop.
...
1. **Check Status**
   Invoke `engram_workflow_status` to inspect the current workflow state and confirm the active task context.
...
3. **Understand and Plan**
   ...
   - For non-trivial tasks, create a brief task plan at `.engram/task-plans/<plan_key>/<phase_key>/<task_key>/task-plan.md` using the deterministic plan, phase, and task keys from the work order, then await user approval before writing code.
